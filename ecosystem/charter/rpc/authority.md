...
- Admission means the application has completed its required authentication, origin, peer, and deployment checks outside RPC.
- After admission, peers establish protocol, transport-representation, required wire-plugin, remote-value, and finite-budget compatibility before application traffic begins.
...
- A peer may operate only on the root and references deliberately issued to that session.
...
- Application admission does not make a peer or its frames trustworthy.
