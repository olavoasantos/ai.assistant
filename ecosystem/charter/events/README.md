...
- A typed payload carrier: each event name maps to a specific payload type through a per-emitter event map.
...
Event payloads are typed through the `EventMap` generic parameter on `EventEmitter<EventMap>` and `Event<Type, Details>`. Rather than a global registry, each consuming subsystem declares its own event map and instantiates emitters against it:
...
A consumer then uses `EventEmitter<ToolEvents>` to get type-safe, locally-scoped event names and payloads. This keeps the foundation generic without declaration merging.
...
- Runtime dependencies are limited to other foundations.
- The contract lives in `@ai.assistant/contracts/events`; source implementations must satisfy it and run shared compliance tests from `@ai.assistant/tests/events`.
